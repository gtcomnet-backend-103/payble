...
final class HandleProviderWebhook implements ShouldQueue
{
    ...
    public function handle(RecordPaymentLedgerPostings $recordLedger): void
    {
        ...
        [$merchantFee, $customerFee] = match ($attempt->paymentIntent->bearer) {
            FeeBearer::Merchant => [$attempt->fee_amount, 0],
            FeeBearer::Customer => [$attempt->fee_amount, 0],
            FeeBearer::Split => [(int) bcmul((string) $attempt->fee_amount, "0.5"), (int) bcmul((string) $attempt->fee_amount, "0.5")],
        };

        $platformFee = PaymentProvider::getFee($attempt->provider, $attempt->channel, $attempt->paymentIntent->amount);
